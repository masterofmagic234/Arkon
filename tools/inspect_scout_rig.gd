...
func _run() -> void:
    ...
    for mesh_node: Node in meshes:
        ...
        var mesh_skeleton: Skeleton3D = null
        if skeletons.size() > 0:
            mesh_skeleton = skeletons[0] as Skeleton3D

        var skin_resource: Skin = mi.get_skin()
        var skeleton_path: NodePath = mi.get_skeleton_path()
        skin_ok = skin_resource != null and mesh_skeleton != null

        print(
            "SKIN INFO: skin_ok=", skin_ok,
            " skeleton_path=", skeleton_path,
            " skeleton=", mesh_skeleton.name if mesh_skeleton != null else "<none>",
            " bind_count=", skin_resource.get_bind_count() if skin_resource != null else 0
        )

        if not skin_ok:
            push_error("SCOUT RIG INSPECT: mesh is not paired with an imported Skeleton3D/Skin")
            quit(1)
            return
