...
const CallableState = preload("res://scripts/callable_state.gd")
...
func _setup_state_machine() -> void:
    ...
    for state_id in [State.PATROL, State.CHASE, State.SEARCH, State.FLEE, State.STUNNED, State.CARRY]:
        state_machine.add_state(state_id, CallableState.new())
    state_machine.transition(state)

func _transition(next_state: int, reason: String) -> void:
    state = next_state
    debug_reason = reason
...
func stun(duration: float = 3.0) -> void:
    _transition(State.STUNNED, "stunned")
    stun_timer = duration
